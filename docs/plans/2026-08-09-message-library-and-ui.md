...
| 12 起 | 上面那个长度 | **载荷 = 完整的 NDC 报文**，含真实控制字符 FS(0x1C)、SO(0x0F)、SI(0x0E) |
| 之后 | 补齐到 614 | 空格填充，丢弃 |
...
⚠️ 键**不是**操作码，是 NCR 自己的场景命名。不要试图拿它和 ATM 发来的操作码配对——控制器实测过，对不上（我们的 `A       `/`C       ` 在库里 0 命中）。它的用途是**给人挑**。
...
- **实时流每帧一行**：方向、字节数、解码后的字段（控制字符渲染成可见符号：FS→`|`、GS→`~`、RS→`^`、SO→`<`、SI→`>`）、命中的规则名。
