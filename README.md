...
键（如 `A A  A A`）里有 912 条**就是操作码**，另有 `GIS`/`OOS`/`PIN1` 这类场景名。
出厂配置的 `opcode-library` 规则就是拿请求第 7 段的操作码直接查库。位语义可从
应答正文反推：`D AB A A` 的凭条写 `FROM CHECKING`/`TO SAVINGS`（位 3 源账户、
位 4 目标账户）、`A A  B A` 写 `NO RECEIPT`（位 6 凭条）、`CBA  A B` 写 `SPANISH`
（位 8 语言）。控制台里显示的是从载荷推出的可读标签。
